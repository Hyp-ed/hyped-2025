...
use hyped_spi::HypedWord::{self, U8};
...
impl<'a, T: HypedSpi> OpticalFlow<'a, T> {
    ...
    pub async fn new(spi: &'a mut T) -> Result<Self, OpticalFlowError> {
        ...
        for offset in 0..NUM_UNIQUE_DATA_VALUES {
            let data = &mut [REG_DATA_READY + HypedWord::U8(offset)];
    ...
    pub async fn get_motion(&mut self) -> Result<(i16, i16), &'static str> {
        ...
        while start.elapsed() < TIMEOUT {
            let mut data = [
                ...
                HypedWord::U8(0x00), // Placeholder for the rest of the 12 bytes
                HypedWord::U8(0x00), // Definitely a better way of doing this but for some reason i was getting syntax errors
                HypedWord::U8(0x00),
                HypedWord::U8(0x00),
                HypedWord::U8(0x00),
                HypedWord::U8(0x00),
            ...
            let x = match (cursor.next(), cursor.next()) {
                (Some(HypedWord::U8(lsb)), Some(HypedWord::U8(msb))) => {
    ...
    async fn bulk_write(&mut self, data: &[HypedWord]) -> Result<(), OpticalFlowError> {
        let mut i = 0;
        ...
        while i < data.len() {
            match data[i] {
                WAIT => {
                    ...
                    if let HypedWord::U8(delay_ms) = data[i + 1] {
                ...
                HypedWord::U8(register) => {
                    ...
                    if let HypedWord::U8(value) = data[i + 1] {
                        let mut words = [HypedWord::U8(register), HypedWord::U8(value)];
                        self.spi
                            .write(&words)
    ...
    async fn secret_sauce(&mut self) -> Result<(), OpticalFlowError> {
        ...
        self.spi
            .transfer_in_place(&mut read_data)
            .map_err(|e| OpticalFlowError::SpiError(e))?;
        ...
        self.bulk_write(&[U8(0x48), U8(value_to_write)]).await?;
        ...
        self.spi
            .transfer_in_place(&mut reg_73_data)
            .map_err(|e| OpticalFlowError::SpiError(e))?;
        if let U8(0x00) = reg_73_data[1] {
            let mut reg_70_data = [U8(0x70), U8(0x00)];
            let mut reg_71_data = [U8(0x71), U8(0x00)];
            self.spi
                .transfer_in_place(&mut reg_70_data)
                .map_err(|e| OpticalFlowError::SpiError(e))?;
            self.spi
                .transfer_in_place(&mut reg_71_data)
                .map_err(|e| OpticalFlowError::SpiError(e))?;
...
// Register Addresses:
const REG_PRODUCT_ID: HypedWord = U8(0x00);
const REG_REVISION_ID: HypedWord = U8(0x01);
const REG_DATA_READY: HypedWord = U8(0x02);
const REG_POWER_UP_RESET: HypedWord = U8(0x3A);
const REG_MOTION_BURST: HypedWord = U8(0x16);
const REG_ORIENTATION: HypedWord = U8(0x5B);
const REG_RESOLUTION: HypedWord = U8(0x4E);
const REG_RAWDATA_GRAB: HypedWord = U8(0x58);
const REG_RAWDATA_GRAB_STATUS: HypedWord = U8(0x59);

const TIMEOUT: Duration = Duration::from_secs(5);
const RETRY_DURATION: Duration = Duration::from_millis(10);

// Register Configurations:
const POWER_UP_RESET_INSTR: HypedWord = U8(0x5A);
const PMW3901_PRODUCT_ID: u8 = 0x49;
const VALID_PMW3901_REVISIONS: [u8; 2] = [0x01, 0x00];

// Sensor Constants:
const NUM_UNIQUE_DATA_VALUES: u8 = 5;
const WAIT: HypedWord = U8(0xFF);
