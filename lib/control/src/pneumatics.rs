...
mod tests {
    ...
    use hyped_sensors::time_of_flight::{
        TimeOfFlightAddresses, RESULT_INTERR_STATUS_GPIO, RESULT_RANGE_VAL, SYS_FRESH_OUT_RESET,
    };

    #[test]
    fn test_pneumatics() {
        embassy_futures::block_on(async move {
            // Create a mock GPIO pin for the brake and lateral suspension
            let brake_pin = MockGpioOutputPin::new();
            let lateral_suspension_pin = MockGpioOutputPin::new();

            // Set up the mock I2C values for the Time of Flight sensor
            ...
            let _ = i2c_values.insert(
                ...
                Some(0), // Time of flight will read 0mm, which is below the threshold for brake actuation
            );
            let i2c_values = Mutex::new(RefCell::new(i2c_values));
            let mut i2c = MockI2c::new(&i2c_values);

            // Create a time of flight sensor for the pneumatics
            let time_of_flight =
                TimeOfFlight::new(&mut i2c, TimeOfFlightAddresses::Address29).unwrap();

            // Create the pneumatics system, passing in the mock GPIO pins and time of flight sensor
            let mut pneumatics = Pneumatics::new(brake_pin, lateral_suspension_pin, time_of_flight)
                .await
                .unwrap();
            ...
            // Disengage brakes
            pneumatics.disengage_brakes().await.unwrap();
            // Modify the time of flight sensor value to be above the threshold for brake actuation
            i2c_values.lock(|values| {
                values
                    .borrow_mut()
                    .insert(
                        (TimeOfFlightAddresses::Address29 as u8, RESULT_RANGE_VAL),
                        Some(10), // Above the threshold for brake actuation
                    )
                    .unwrap();
            });
            ...
            // Engage brakes
            pneumatics.engage_brakes().await.unwrap();
            // Modify the time of flight sensor value to be below the threshold for brake actuation
            i2c_values.lock(|values| {
                values
                    .borrow_mut()
                    .insert(
                        (TimeOfFlightAddresses::Address29 as u8, RESULT_RANGE_VAL),
                        Some(0), // Below the threshold for brake actuation
                    )
                    .unwrap();
            });
            assert_eq!(pneumatics.get_brake_state(), BrakeState::Engaged);
