...
import type { Datum } from 'openmct/types/Datum';
import type { Unpacked } from 'openmct/types/Unpacked';
...
export function LimitPlugin() {
	return function install(openmct: OpenMCT) {
		const provider = {
			...
			supportsLimits: (domainObject: AugmentedDomainObject) =>
				domainObject.limits !== undefined,
			/**
			 ...
			 */
			getLimits: (domainObject: AugmentedDomainObject) => {
