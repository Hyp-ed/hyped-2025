...
use embassy_stm32::can::{
    filter::Mask32, frame::Frame, Can, CanTx, Fifo, Rx0InterruptHandler, Rx1InterruptHandler,
...
pub async fn send_can_message(tx: &'static mut CanTx<'static>) {
    ...
    loop {
        let frame = Frame::new_data(
            unwrap!(StandardId::new(2)),
            &[0xde, 0xad, 0xbe, 0xef, counter as u8],
        )
        .unwrap();
        tx.write(&frame).await;
        embassy_time::Timer::after_secs(1).await;
        counter += 1;
        info!("Sent {} frames", counter);
    }
}
...
async fn main(spawner: Spawner) {
    ...
    loop {
        ...
        let counter_value = envelope.frame.data()[4];
        info!("Received frame with counter value: {}", counter_value);
    }
}
